...
const Parallax=({type})=>{
    ...
    const yBg=useTransform(scrollYProgress, [0,1], ["50%","-890%"]);
    const yPl=useTransform(scrollYProgress, [0,1], ["100%","-400%"]);
    const ySt=useTransform(scrollYProgress, [0,1], ["0%","15%"]);
    const ySs=useTransform(scrollYProgress, [0,1], ["50%","-950%"]);
    ...
    return (
       ...
       <div className="parallax" style={{background:type==="services" ? "linear-gradient(180deg,#111132,#0c0c1d" : "linear-gradient(180deg,#111132,#505064"}}>
        <div className="hh1"><motion.h1 
            style={{y: type==="services" ? yBg : ySs,marginTop:t3 === t2 ? "400px" : "0px" }}>
            {t3}
            </motion.h1></div>
        <motion.div className="mountains"></motion.div>
        <motion.div style={{x:ySt}} className="shooting"></motion.div>
        <motion.div className="planets" 
        style={{y: type==="services" ? yBg : yPl, 
        backgroundImage: (p3),}}>
        </motion.div>
